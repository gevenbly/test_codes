...
for p in range(n_iterations):
  ...
  for z in range(n_levels):
    ...
    if p > 10:
      ...
      u[z] = (-1 * utemp @ vtemph).reshape(uSize)
      # -----------------
    ...
    w[z] = (-1 * utemp @ vtemph).reshape(wSize)
...
np.save('XXDataS0.npy', (u, w, v, rho, ham))
# u, w, v, rho, ham = np.load('XXDataS0.npy', allow_pickle=True)
